...
ggplot(nhanes, aes(x = sex1)) + geom_bar()
ggplot(nhanes, aes(x = sex1, fill = sex1, color = sex1)) + geom_bar()

# horizontal
ggplot(nhanes, aes(x = sex1, fill = sex1, color = sex1)) + geom_bar() + coord_flip()
```
...
ggplot(nhanes, aes(x = age)) + geom_histogram()
ggplot(nhanes, aes(x = age)) + geom_histogram(binwidth = 10)

# add density plot
ggplot(nhanes, aes(x = sbp, ..density..)) + geom_histogram(fill = "lightgreen", color = "green", binwidth = 15) + geom_density()

# overlapping histograms
ggplot(nhanes, aes(x = sbp, fill = sex1)) + geom_histogram() + facet_grid(sex1 ~ .)
ggplot(nhanes, aes(x = sbp, fill = sex1)) + geom_histogram(binwidth = 10)
ggplot(nhanes, aes(x = sbp, fill = sex1)) + geom_histogram(binwidth = 10, position="identity", alpha=0.8) + scale_fill_grey(start = 0, end = 0.75)
```
...
ggplot(nhanes, aes(x = log(bpb))) + geom_boxplot(fill = "#990000",  color = "#3366FF", notch = T, notchwidth = .3)

# boxplot split by another variable
ggplot(nhanes, aes(x = AGE5b, y = log(bpb))) + geom_boxplot()

# boxplot split by two variables
ggplot(nhanes, aes(x = AGE5b, y = log(bpb))) + geom_boxplot(aes(fill = sex1)) + labs(title = "Boxplot of Blood Lead Levels") + scale_x_discrete("Age") + scale_y_continuous("Log(Blood lead levels)")
```
...
ggplot(results, aes(x = or, y = x1,  xmin = or.ll, xmax = or.ul)) + geom_pointrange() + xlab("Odds Ratio") + ylab("Group") + geom_vline(xintercept = 1, linetype = 2)  
```
...
abline(lm( log(nhanes$bpb) ~ nhanes$age), col = "firebrick1", lwd = 3)
...
dim(nomiss)
head(nomiss)
plot(nomiss$nhanes.age, nomiss$nhanes.bmi, xlab = "Age", ylab = "BMI", las = 1, cex.lab = 1.2, pch = 20, cex = 0.6)
title("Scatterplot of Age vs. BMI")
...
plot(nhanes$age, nhanes$bmi, xlab = "Age", ylab = "BMI", las = 1, cex.lab = 1.2, pch = 20, cex = 0.6)
title("Scatterplot of Age vs. BMI")
...
plot(nhanes$age, log(nhanes$bpb), xlab = "Age", ylab = "Log(Blood lead)", pch = nhanes$sex, col = c("seagreen", "darkorchid")[nhanes$sex], cex = 0.3, las = 1, cex.lab = 1.2)
abline(lm(log(nhanes$bpb)[nhanes$sex == 1] ~ nhanes$age[nhanes$sex == 1]), lty = 1, lwd = 3, col = "seagreen")
abline(lm(log(nhanes$bpb)[nhanes$sex == 2] ~ nhanes$age[nhanes$sex == 2]), lty = 2, lwd = 3, col = "darkorchid")
legend("topleft", c("Male", "Female"), lty = c(1, 1), pch = c(1, 2), col = c("seagreen", "darkorchid"), bty="n")
...
ggplot(nhanes, aes(x = age, y = log(bpb))) + geom_point(shape = 1) # open circles

## add a linear fit
ggplot(nhanes, aes(x = age, y = log(bpb))) + geom_point() + geom_smooth(method = lm)

## add a smoothing line
ggplot(nhanes, aes(x = age, y = log(bpb))) + geom_point() + geom_smooth()
...
abline(lm(log(nhanes$bpb) ~ nhanes$age), col = "firebrick1", lwd = 2)
boxplot(log(nhanes$bpb) ~ nhanes$AGE5b, notch = T, las = 2, ylab = "Log(Blood Lead Level)", pch = 20, cex = 0.5, xlab="Age")
par(mfrow = c(1, 1)) # Go back to 1 row of graphs with 1 column of graphs
## In traditional graphics, just do the graphs in order and they find their place
```


```{r multiple_plots_one_page_ggplot2}
## In grid, we must instead save the plot to an object
plot1 <- ggplot(nhanes, aes(x = sex1, fill = sex1, color = sex1)) + geom_bar()
plot2 <- ggplot(nhanes, aes(x = sbp, ..density..)) + geom_histogram(fill = "lightgreen", color = "green", binwidth = 15) + geom_density()
plot3 <- ggplot(nhanes, aes(x = AGE5b, y = log(bpb))) + geom_boxplot()
plot4 <- ggplot(nhanes, aes(x = age, y = log(bpb))) + geom_point(size = 0.2, alpha = 0.5) + geom_smooth()
...
# Put it all together
pairs(cbind(nhanes$sbp, nhanes$bpb, nhanes$bmi, nhanes$age), main = "Pairwise Relationships", pch = 20, cex = 0.4, font.labels = 2, cex.labels = 2, col = "dodgerblue", upper.panel = panel.cor, diag.panel = panel.hist, lower.panel = panel.smooth, labels=c("sbp", "bpb", "bmi", "age"))
```

```{r scatterplot_matrix_ggplot2}
# Scatterplot matrix, ggplot2 package
ggpairs(nhanes[, c("sbp", "dbp", "bmi", "bpb")])
```
